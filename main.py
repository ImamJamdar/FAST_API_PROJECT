...
def sort_rooms(
    ...
    sorted_rooms = sorted(rooms, key=lambda r: r[sort_by], reverse=reverse)
...
def browse_rooms(
    ...
    reverse = order == "desc"
    result = sorted(result, key=lambda r: r[sort_by], reverse=reverse)
...
def add_room(new_room: NewRoom, response: Response):
    for r in rooms:
        if r["room_number"] == new_room.room_number:
